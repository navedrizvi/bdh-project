...
def subnotes (note: str):    
    ...
    #2
    #filter = "medical illness"

    #3
    #filter = "past medical history"

    #4
    filter = "medication"

    #5
    #filter = "past surgical history"
...
def preprocess(sample_ids: set) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    #get subnotes first
    notes_preprocessed['SUBNOTES'] = notes_preprocessed['TEXT'].map(subnotes)

    #notes_preprocessed['CLEAN_TEXT'] = notes_preprocessed['TEXT'].map(clean_text)
    notes_preprocessed['CLEAN_TEXT'] = notes_preprocessed['SUBNOTES'].map(clean_text)
